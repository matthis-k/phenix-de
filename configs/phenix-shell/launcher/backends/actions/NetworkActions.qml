        ...
        return (VpnService.destinations || []).slice().sort(function(a, b) {
            ...
            return {
                ...
                display: {
                    ...
                    subtitle: destination.value,
                    icon: "network-vpn-symbolic",
                    iconColor: destination.kind === "group" ? Config.styling.info : Config.styling.good
                },
                match: { aliases: [destination.name] },
                action: { service: "vpn", op: "connect", destination: destination.value }
            };
        });
    }
